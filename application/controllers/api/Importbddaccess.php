...
class Importbddaccess extends CI_Controller {
	...
	public function importerbasededonnees() {
		...
		foreach($rowIterator as $row) {
			...
			if($ligne >=2) {
				...
				if(isset($libelle) && $libelle >"" && isset($code) && $code >"") {
					$libelle = strtolower($libelle);
		...
		foreach($rowIterator as $row) {
			...
			if($ligne >=2) {
				...
				if(isset($code) && $code >"" && isset($nom) && $nom >"" ) {
					$nom = strtolower($nom);
		...
		foreach($rowIterator as $row) {
			...
			if($ligne >=2) {
				...
				if(isset($nom_local) && $nom_local >"" && isset($nom_scientifique) && $nom_scientifique >"" && isset($code) && $code >"" ) {
		...
		foreach($rowIterator as $row) {
			...
			if($ligne >=2) {
				...
				if(isset($nom) && $nom >"" && isset($region) && $region >"" && isset($site) && $site >"") {
					$nom = strtolower($nom);
		...
		foreach($rowIterator as $row) {
			...
			if($ligne >=2) {
				...
				if(isset($Code3Alpha) && $Code3Alpha >"" && isset($code_unique_espece) && $code_unique_espece >"") {
					if($validation=="VRAI") {
						$validation=1;
					} else {
						$validation=0;
	...
	public function importerbasededonneesenquetecadre() {
		require_once 'Classes/PHPExcel.php';
		require_once 'Classes/PHPExcel/IOFactory.php';
		set_time_limit(0);
		$repertoire= $_POST['repertoire'];
		$chemin=dirname(__FILE__) . "/../../../../".$repertoire;
		// $retour=$this->ImportbddaccessManager->RAZ_enquete_cadre();		
		$lien_vers_mon_document_excel = $chemin . "enquete_cadre.xlsx";
		if(strpos($lien_vers_mon_document_excel,"xlsx") >0) {
			// pour mise à jour après : G4 = id_fiche_paiement <=> déjà importé => à ignorer
			$objet_read_write = PHPExcel_IOFactory::createReader('Excel2007');
			$excel = $objet_read_write->load($lien_vers_mon_document_excel);			 
			...
			if($ligne >=2) {
				...
				foreach ($cellIterator as $cell) {
					if('A' == $cell->getColumn()) {
						...
						$nbr_unite_peche =$cell->getValue();
					 } else if('G' == $cell->getColumn()) {
						$unique_code =$cell->getValue();
					 } 
				}
				if(isset($region) && $region >"" && isset($site) && $site >"" && isset($annee) && $annee >"" && isset($engin_canoe) && $engin_canoe >"") {
					$region = strtolower($region);
					$site = strtolower($site);
					$remplacer=array("");
					$trouver= array(" ");
					...
					if($retour) {
						foreach($retour as $k=>$v) {
							if($k==0) {
								...
								$ajout=$v->ajout;
								if($ajout_canoe=="ajout") {
									$liste_type_canoe[$t_c]["id"]=$id_type_canoe;
									$liste_type_canoe[$t_c]["code"]=$code_canoe;
									$liste_type_canoe[$t_c]["nom"]=$nom_canoe;
									$entre_canoe="tao ka";
									$t_c=$t_c + 1;
								}
								if($ajout_engin=="ajout") {
									$liste_type_engin[$t_e]["id"]=$id_type_engin;
									$liste_type_engin[$t_e]["code"]=$code_engin;
									$liste_type_engin[$t_e]["libelle"]=$libelle_engin;
									$entre_engin="tao izy ka";
									$t_e=$t_e + 1;
								}
							}
					...
					$retour=$this->ImportbddaccessManager->AjouterEnqueteCadre($annee,$id_region,$id_site_embarquement,$id_unite_peche,$nbr_unite_peche);
					if($retour) {
						foreach($retour as $k=>$v) {
							if($k==0) {
								$liste_enquete_cadre[$e_c]["id"]=$v->id;
								$liste_enquete_cadre[$e_c]["id_region"]=$v->id_region;
								$liste_enquete_cadre[$e_c]["region"]=$region;
								$liste_enquete_cadre[$e_c]["id_site_embarquement"]=$v->id_site_embarquement;
								$liste_enquete_cadre[$e_c]["id_unite_peche"]=$v->id_unite_peche;
								$liste_enquete_cadre[$e_c]["nbr_unite_peche"]=$v->nbr_unite_peche;
								$liste_enquete_cadre[$e_c]["annee"]=$v->annee;
								$e_c=$e_c + 1;
							}
						}
					}
